# Copied from https://github.com/pixeltable/pixeltable/blob/v0.4.0/tool/mypy_plugin.py
# it allows mypy to understand Pixeltable's custom types and decorators,
...
class PxtPlugin(Plugin):
    ...
    __ARRAY_GETITEM_FULLNAME = (
        f"{pxt.Array.__module__}.{pxt.Array.__name__}.__class_getitem__"
    )
    __ADD_COLUMN_FULLNAME = (
        f"{pxt.Table.__module__}.{pxt.Table.__name__}.{pxt.Table.add_column.__name__}"
    )
    __ADD_COMPUTED_COLUMN_FULLNAME = f"{pxt.Table.__module__}.{pxt.Table.__name__}.{pxt.Table.add_computed_column.__name__}"
    ...
    __FULLNAME_MAP: ClassVar[dict] = {
        f"{k.__module__}.{k.__name__}": v for k, v in __TYPE_MAP.items()
    }
    ...
    def get_method_signature_hook(
        ...
        if fullname in (
            self.__ADD_COLUMN_FULLNAME,
            self.__ADD_COMPUTED_COLUMN_FULLNAME,
        ):
...
def adjust_kwargs(ctx: MethodSigContext) -> FunctionLike:
    ...
    return sig.copy_modified(
        arg_names=new_arg_names, arg_types=new_arg_types, arg_kinds=new_arg_kinds
    )
...
def adjust_uda_methods(ctx: ClassDefContext) -> bool:
    ...
    fn_arg = nodes.Argument(
        nodes.Var("fn"), AnyType(TypeOfAny.special_form), None, nodes.ARG_POS
    )
    args_arg = nodes.Argument(
        nodes.Var("args"), AnyType(TypeOfAny.special_form), None, nodes.ARG_STAR
    )
    kwargs_arg = nodes.Argument(
        nodes.Var("kwargs"), AnyType(TypeOfAny.special_form), None, nodes.ARG_STAR2
    )
    add_method_to_class(
        ctx.api,
        ctx.cls,
    ...
    add_attribute_to_class(
        ctx.api, ctx.cls, "signatures", typ=list_type, is_classvar=True
    )
    return True
